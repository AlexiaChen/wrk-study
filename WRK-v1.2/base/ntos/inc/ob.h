...
/*
可以看出:
在调用ObCreateObjectType函数来构建一种新的对象类型是，调用者除了可以指定此
种类对象的一些数据特性外，还可以指定该类型对象的一些基本操作方法，
包括: Dump, Open ,Close,  Delete,Parse,Security,QueryName和OkayToClose。


对象管理器正是通过这些方法来统一管理各种类型的对象的。


系统有一个全局变量ObpObjectTypes数组记录了所有已创建的类型,这个是一个静态
数组(在Obp.h中定义的这个数组).  WRK限定不超过48种对象类型。OBJECT_TYPE中的Index
成员记录了一个类型对象在此数组中的索引
...
typedef struct _OBJECT_TYPE_INITIALIZER {
    ...
    OB_DUMP_METHOD DumpProcedure;                      //指定该类型对象的一些操作方法
...
/*
系统定义的对象种类是有限的，WRK支持31种对象，各有一个全局的POBJECT_TYPE
变量指向其类型对象.


为了使每一种对象都可以在系统内部真正可用，系统通常会在初始化过程中
调用ObCreateObjectType函数构建这种对象类型，以完成相应的全局变量的初始化.
*/
...
//构建对象的类型,以完成相应的全局变量的初始化
NTKERNELAPI
NTSTATUS
ObCreateObjectType(
    __in PUNICODE_STRING TypeName,
    __in POBJECT_TYPE_INITIALIZER ObjectTypeInitializer,   //注意这个输入参数
...
//由以上英文看出，居然还建议编译器尽量省去call指令了.
//可能考虑到系统会频繁的创建,访问输出对象吧
